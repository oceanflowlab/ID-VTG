...
def ctr_giou_loss(
    ...
    assert (input_offsets >= 0.0).all(), f"predicted offsets must be non-negative. Min value: {input_offsets.min()}, Max value: {input_offsets.max()}"
    assert (target_offsets >= 0.0).all(), f"GT offsets must be non-negative. Min value: {target_offsets.min()}, Max value: {target_offsets.max()}"
