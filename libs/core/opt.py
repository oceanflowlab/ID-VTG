...
def _merge(src, dst):
    ...
    for k, v in src.items():
        if k in dst:
            if isinstance(v, dict) and dst[k] is not None:
                _merge(src[k], dst[k])
        else:
            dst[k] = v
...
def _update_opt(opt, is_training=True):
    ...
    opt['model']['visual_fusion']['text_dim'] = text_dim
    opt['model']['visual_fusion']['vid_dim'] = vid_dim
    opt['model']['text_fusion']['text_dim'] = text_dim
    opt['model']['text_fusion']['vid_dim'] = vid_dim
